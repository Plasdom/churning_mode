...
Field3D Churn::div_q_par_classic(const Field3D &T, const Field3D &K_par, const Vector3D &b)
{
    ...
    BOUT_FOR(i, mesh->getRegion3D("RGN_NOBNDRY"))
    {
        ...
        else if (fixed_P_core)
        {
            if ((mesh->getGlobalYIndex(i.y()) >= mesh->GlobalNy - ngcy_tot - 1) && psi(i.x(), mesh->GlobalNy - ngcy_tot, i.z()) < psi_bndry_P_core_BC)
            {
                A_plus_half = 0.0;
            }
        }
...
Field3D Churn::div_q_par_gunter(const Field3D &T, const Field3D &K_par, const Vector3D &b)
{
    ...
    else if (fixed_P_core)
    {
        for (int i = 0; i < mesh->LocalNx; i++)
        {
            if (mesh->lastY(i) && psi(i, mesh->GlobalNy - ngcy_tot, 0) < psi_bndry_P_core_BC)
            {
                q_pary_corners(i, mesh->LocalNy - ngcy_tot, 0) = 0.0;
            }
        }
...
Field3D Churn::Q_plus(const Field3D &u, const Field3D &K_par, const Vector3D &b)
{
    ...
    for (auto i : result)
    {
        ...
        {
            if ((mesh->getGlobalYIndex(i.y() + n_y + 1) > mesh->GlobalNy - ngcy_tot - 1) && (psi(i.x(), mesh->GlobalNy - ngcy_tot, i.z()) < psi_bndry_P_core_BC))
            {
                u_plus = u[i];
            }
        }
...
Field3D Churn::Q_minus(const Field3D &u, const Field3D &K_par, const Vector3D &b)
{
    ...
    for (auto i : result)
    {
        ...
        {
            if ((mesh->getGlobalYIndex(i.y() - n_y) > mesh->GlobalNy - ngcy_tot - 1) && (psi(i.x(), mesh->GlobalNy - ngcy_tot, i.z()) < psi_bndry_P_core_BC))
            {
                u_minus = u[i];
            }
        }
